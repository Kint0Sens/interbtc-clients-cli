    ...
    match &cli.command {
        Commands::Vaults => {
            ...
            for vault in vaults.iter() {
                ...
                println!("   Tokens (To be issued / To be redeemed / To be replaced) {} {} {}", to_be_issued_tokens_str, to_be_redeemed_tokens_str, to_be_replaced_tokens_str);
        ...
        Commands::LiquidationVault => {
            ...
            if  liquidation_vault.currency_pair.collateral.inner().unwrap().decimals() > liquidation_vault.currency_pair.wrapped.inner().unwrap().decimals()  {
                let decimals = liquidation_vault.currency_pair.collateral.inner().unwrap().decimals() as u32
                - liquidation_vault.currency_pair.wrapped.inner().unwrap().decimals() as u32;
            ...
            } else if liquidation_vault.currency_pair.collateral.inner().unwrap().decimals() < liquidation_vault.currency_pair.wrapped.inner().unwrap().decimals()  {
                let decimals = liquidation_vault.currency_pair.wrapped.inner().unwrap().decimals() as u32
                - liquidation_vault.currency_pair.collateral.inner().unwrap().decimals() as u32;
                // let user_receives_per_token_decimals = 10_u128.pow(decimals); 
                let user_receives_per_token_decimals_f = 10_f32.powf(decimals as f32); 
            ...
            table
                .add_heading(format!("Liquidation Vault [{}/{}]",
                    liquidation_vault.currency_pair.collateral.inner().unwrap().symbol(),
                    liquidation_vault.currency_pair.wrapped.inner().unwrap().symbol()
                    )
                )
                .add_heading("--------------------------------")
                .add_row(Row::new()
                    ...
                    .with_cell(liquidation_vault.currency_pair.collateral.inner().unwrap().symbol())
                    .with_cell("")
                )
                .add_row(Row::new()
                    .with_cell("Burnable")
                    .with_cell(burnable_str)
                    .with_cell(liquidation_vault.currency_pair.wrapped.inner().unwrap().symbol())
